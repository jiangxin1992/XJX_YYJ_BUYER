 ...
 * 新增一条日活记录
 */
+ (void)addStatPVWithOpusDetailId:(NSString *)opusDetailId{
    // get URL
    NSString *requestURL = [[[NSUserDefaults standardUserDefaults] objectForKey:kLastYYServerURL] stringByAppendingString:KBurideStatPv];
    // 传入的值都没用
    NSDictionary *dic = [YYHttpHeaderManager buildHeadderWithAction:KBurideStatPv params:nil];

    NSDictionary *parameters = @{@"platform":@"IOS",@"cat":@"STYLE",@"key":opusDetailId};
    NSData *body = [parameters mj_JSONData];
