...
@implementation YYBuyerModifyInfoUploadViewCell{
    NSInteger itemWidth;
...
-(void)updateCellInfo:(NSArray*)info{
    ...
    if(itemWidth == 0 ){
        itemWidth = (SCREEN_WIDTH - 17*2 -15*3)/4;
    ...
    for (int i =0; i<8; i++) {
        ...
        if(i == imgnum ){
            ...
            _tipViewLayoutTopConstraint.constant = 57 +(i/4)*(tipViewHeight+itemspace);
            _tipViewLayoutLeftConstraint.constant = 17 + (i%4)*(tipViewWidth+itemspace);
...
+(float)cellHeight:(NSInteger)count{
    NSInteger itemWidth = (SCREEN_WIDTH - 17*2 -15*3)/4; //top 15 bottom 39
    
    return (15 + itemWidth)*((count/4)+1) +50;
}
#pragma mark - Other
