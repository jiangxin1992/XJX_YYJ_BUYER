...
NSString *getMD5String(NSString *str)
{
    if(str && [str length])
    {
        ...
        return [NSString stringWithCString:(const char *)resultHEX encoding:NSUTF8StringEncoding];
    }
...
{
    int nLen = 0;
    if(str && (nLen = (int)[str length]))
    {
        char *srcString = malloc(nLen + 1);
        if(srcString)
        {
            memset(srcString, 0, nLen + 1);
            [str getCString:srcString maxLength:nLen encoding:NSUTF8StringEncoding];
