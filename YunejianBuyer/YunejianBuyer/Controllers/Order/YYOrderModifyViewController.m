...
@interface YYOrderModifyViewController ()<UITableViewDataSource,UITableViewDelegate,UIPopoverControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,YYTableCellDelegate>
...
- (void)updateUI{
    ...
    if (_isCreatOrder) {
        _navigationBarViewController.nowTitle = NSLocalizedString(@"确认订单",nil);
...
- (IBAction)saveButtonClicked:(id)sender{
    ...
    if(adress != nil ){
        ...
        [YYOrderApi createOrModifyAddress:adress orderCode:orderCode andBlock:^(YYRspStatusAndMessage *rspStatusAndMessage, YYBuyerAddressModel *addressModel, NSError *error) {
            if (rspStatusAndMessage.status == kCode100 && addressModel && addressModel.addressId){
...
#pragma mark --款式添加折扣
- (void)addAstyleDiscount:(YYOrderModifyViewController *)weakSelf andOrderStyleModel:(YYOrderStyleModel *)orderStyleModel AndSeriesId:(long)seriesId {
}
