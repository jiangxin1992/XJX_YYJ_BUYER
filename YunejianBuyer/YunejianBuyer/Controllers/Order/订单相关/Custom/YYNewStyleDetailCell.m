...
@property (nonatomic, weak) IBOutlet NSLayoutConstraint *bottomFooterHeightConstraint;
...
@property (nonatomic, weak) IBOutlet UIView *detailBackgroundView;
...
@property (nonatomic, weak) IBOutlet UILabel *stylePriceTitle;
...
@property (weak, nonatomic) IBOutlet UILabel *sizeNameTitle;
@property (weak, nonatomic) IBOutlet UILabel *totalPriceTitle;
@property (weak, nonatomic) IBOutlet UILabel *receivedTitle;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *sizeNameTitleRightWidthLayout;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *totalPriceTitleRightWidthLayout;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *receivedTitleRightWidthLayout;
...
- (void)updateUI{
    ...
    /*颜色、尺码等标题及显示位置*/
    _receivedTitle.hidden = !_isReceived;
    if(_isReceived){
        _sizeNameTitleRightWidthLayout.constant = -184;
        _totalPriceTitleRightWidthLayout.constant = -114;
    }else{
        if (self.style != YYNewStyleInventory) {
            _sizeNameTitleRightWidthLayout.constant = -170;
            _totalPriceTitleRightWidthLayout.constant = -62;
        }
    }
    _totalPriceTitle.hidden = NO;
    if (self.style == YYNewStyleInventory) {
        self.totalPriceTitle.text = NSLocalizedString(@"库存数", nil);
        self.receivedTitle.text = NSLocalizedString(@"类型", nil);
    }else if (self.style == YYNewStyleWarehousingRecord) {
        self.totalPriceTitle.text = NSLocalizedString(@"入库数", nil);
        self.receivedTitle.text = @"";
    }else if (self.style == YYNewStyleEXwarehousingRecord) {
        self.totalPriceTitle.text = NSLocalizedString(@"出库数", nil);
        self.receivedTitle.text = @"";
    }else if (self.style == YYNewStylePackageDetail) {
        self.totalPriceTitle.text = NSLocalizedString(@"本次发货数", nil);
        self.receivedTitle.text = @"";
    }else {
        self.totalPriceTitle.text = NSLocalizedString(@"件数", nil);
        self.receivedTitle.text = NSLocalizedString(@"确认收货", nil);
    }
    /*********************/
    ...
    if (self.delegate == nil || _isModifyNow == 0 || self.style == YYNewStyleInventory || self.style == YYNewStyleWarehousingRecord || self.style == YYNewStyleEXwarehousingRecord || self.style == YYNewStylePackageDetail) {
    ...
    if (self.style == YYNewStyleWarehousingRecord || self.style == YYNewStyleEXwarehousingRecord) {
        self.detailBackgroundView.backgroundColor = _define_white_color;
        
        NSString *imageRelativePath = self.warehouseStyleModel.styleImg;
        sd_downloadWebImageWithRelativePath(NO, imageRelativePath, _styleImage, kStyleCover, 0);
        self.styleNameLabel.text = self.warehouseStyleModel.styleName;
        self.styleCodeLabel.text = [NSString stringWithFormat:NSLocalizedString(@"款号：%@\n品牌：%@", nil), self.warehouseStyleModel.styleCode, self.warehouseStyleModel.brandName];
    }else {
        NSString *imageRelativePath = _orderStyleModel.albumImg;
        sd_downloadWebImageWithRelativePath(NO, imageRelativePath, _styleImage, kStyleCover, 0);
        _styleNameLabel.text = _orderStyleModel.name;
        self.styleCodeLabel.text = _orderStyleModel.styleCode;
    }
    ...
    }else if (self.style == YYNewStyleInventory || self.style == YYNewStyleWarehousingRecord || self.style == YYNewStyleEXwarehousingRecord) {
        self.stylePriceTitle.hidden = YES;
        self.styleTaxPriceTitle.hidden = YES;
        self.stylePriceLabel.hidden = YES;
        self.styleTaxPriceLabel.hidden = YES;
        self.priceDiscountView.hidden = YES;
    } else{
    ...
    if((_isModifyNow != 4 ) && needPayTaxView([_orderStyleModel.curType integerValue]) ){
        if(_selectTaxType){
            ...
            float taxRate =[getPayTaxValue(_menuData, _selectTaxType, NO) doubleValue];
    ...
    if (self.style == YYNewStyleInventory || self.style == YYNewStyleWarehousingRecord || self.style == YYNewStyleEXwarehousingRecord) {
        self.bottomFooterHeightConstraint.constant = 0;
        self.bottomLineLayoutConstraint.constant = 0;
        self.amountMinTipLabel.hidden = YES;
    }
    /*********************/
    ...
    if (self.style != YYNewStyleInventory && self.style != YYNewStyleWarehousingRecord && self.style != YYNewStyleEXwarehousingRecord && self.style != YYNewStylePackageDetail) {
        /*提示信息*/
        if(!_noCountTipView){
            //创建提示view
            int top_magin = 20;
...
+ (float)CellHeight:(NSInteger)num showHelpFlag:(BOOL)isShow showTopHeader:(BOOL)isShowHeader showBottomFooter:(BOOL)isShowFooter {
    ...
    return (isShowHeader ? 285 : 245) - 42 + top_magin + (num?(top_magin+item_height)*num:(top_magin+item_height)) + (isShow ? 15 : 0) - (isShowFooter ? 0 : 45);
}
...
- (void)addSimpleAlineView {
    int item_width = SCREEN_WIDTH;
    int item_height = 30;
    int top_magin = 20;
    BOOL isFirstTime = YES;
    for (YYWarehouseSizeModel *size in self.warehouseStyleModel.sizes) {
        ...
        if (!sizeInfoView) {
            ...
            [self.oneSizeInfoContainer addSubview:sizeInfoView];
        }
        ...
        sizeInfoView.style = self.style;
        sizeInfoView.hiddenColor = (!isFirstTime && (self.style == YYNewStyleWarehousingRecord || self.style == YYNewStyleEXwarehousingRecord));
        sizeInfoView.isReceived = NO;
        sizeInfoView.isColorSelect = @(NO);
        sizeInfoView.hidden = NO;
        [sizeInfoView setInputBoxHide:YES];
        sizeInfoView.warehouseStyleModel = self.warehouseStyleModel;
        sizeInfoView.warehouseSizeModel = size;
        ...
        if(sizeCount){
            for (NSInteger i = (sizeCount-1); i >=0 ; i--) {
                ...
                if((tmpAmount > 0 || tmpAmount == -1) || isColorSelect){
                    ...
                    sizeInfoView.style = self.style;
                    sizeInfoView.hiddenColor = (!isFirstTime && (self.style == YYNewStyleWarehousingRecord || self.style == YYNewStyleEXwarehousingRecord));
                    sizeInfoView.isReceived = _isReceived;
