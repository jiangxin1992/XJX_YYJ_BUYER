...
@property (nonatomic, strong) UIButton *taxTypeBtn;
...
-(void)createNavigationBarView{
    self.navView = [[YYNavView alloc] initWithTitle:NSLocalizedString(@"购物车",nil) WithSuperView:self.view haveStatusView:YES];
    WeakSelf(ws);
    self.navView.goBackBlock = ^{
        [ws goBack];
    };
    [self initFinishEditButton];
    [self initEditButton];
    [self initTaxTypeButton];
}
#pragma mark - --------------系统代理----------------------
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [YYNewStyleDetailCell CellHeight:styleBuyedSizeCount showHelpFlag:showMinAmount showTopHeader:NO showBottomFooter:YES];//210 + styleModel.amount.count * 50;
}
...
- (void)editBtnAction:(id)sender {
    ...
    [self.navView setNavTitle:NSLocalizedString(@"编辑购物车",nil)];
    [self.tableView reloadData];
}

#pragma mark -完成编辑
- (void)finishEditBtnAction:(id)sender {
    //隐藏编辑状态下的按钮
    ...
    [self.navView setNavTitle:NSLocalizedString(@"购物车",nil)];
    //改变约束
    if(sender != nil){
...
#pragma mark -选择税率按钮
- (void)showTaxTypeMenu:(id)sender {
    [self finishEditBtnAction:sender];
    //跳转税率选择界面
...
#pragma mark - --------------自定义方法----------------------
- (void)initFinishEditButton {
    self.finishEditBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    [self.finishEditBtn setTitle:NSLocalizedString(@"完成", nil) forState:UIControlStateNormal];
    [self.finishEditBtn setTitleColor:_define_black_color forState:UIControlStateNormal];
    self.finishEditBtn.titleLabel.font = [UIFont systemFontOfSize:16];
    self.finishEditBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    [self.finishEditBtn addTarget:self action:@selector(finishEditBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    self.finishEditBtn.hidden = YES;
    [self.navView addSubview:self.finishEditBtn];
    [self.finishEditBtn mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)initEditButton {
    ...
    [self.editBtn setTitleColor:_define_black_color forState:UIControlStateNormal];
...
- (void)initTaxTypeButton {
    self.taxTypeBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    [self.taxTypeBtn setTitle:NSLocalizedString(@"税制", nil) forState:UIControlStateNormal];
    [self.taxTypeBtn setTitleColor:_define_black_color forState:UIControlStateNormal];
    self.taxTypeBtn.titleLabel.font = [UIFont systemFontOfSize:16];
    self.taxTypeBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    [self.taxTypeBtn addTarget:self action:@selector(showTaxTypeMenu:) forControlEvents:UIControlEventTouchUpInside];
    [self.navView addSubview:self.taxTypeBtn];
    WeakSelf(ws);
    [self.taxTypeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(80);
        make.height.mas_equalTo(44);
        make.bottom.mas_equalTo(-1);
        make.right.mas_equalTo(ws.editBtn.mas_left);
    }];
}
...
-(void)showBuildOrderUI{
    ...
    AppDelegate *appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    orderInfoModel.orderConnStatus = [[NSNumber alloc] initWithInteger:YYOrderConnStatusUnconfirmed];
    orderInfoModel.designerOrderStatus =[[NSNumber alloc] initWithInteger:YYOrderCode_NEGOTIATION];
    orderInfoModel.buyerOrderStatus =[[NSNumber alloc] initWithInteger:YYOrderCode_NEGOTIATION];
...
-(void)checkStyleModify:(YYOrderInfoModel *)orderInfoModel callBack:(void (^)())block{
    ...
    WeakSelf(ws);
    __block NSMutableDictionary *blockStyleInfo = styleInfo;
