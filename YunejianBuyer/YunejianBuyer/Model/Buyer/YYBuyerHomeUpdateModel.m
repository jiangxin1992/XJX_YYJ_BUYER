...
@implementation YYBuyerHomeUpdateModel
+(YYBuyerHomeUpdateModel *)createUploadCertModel:(NSArray *)paramsArr{
    YYBuyerHomeUpdateModel *model = [YYBuyerHomeUpdateModel new];
    NSString *key = nil;
    NSString *value = nil;
    ...
    for (NSString *info in paramsArr) {
        ...
        if([infoArr count] > 1 && ![NSString isNilOrEmpty:key]){
            ...
            }else if([key containsString:@"copBrands"]){
                model.copBrands = dictionaryWithJsonString(value);
            }else{
