...
- (instancetype)initViewWithFrame:(CGRect)frame autoPlayTime:(NSTimeInterval)playTime type:(NSString *)type imagesArray:(NSArray *)imagesArray  clickCallBack:(void (^)(NSInteger))clickCallBack
{
...
- (void)creatUI
{
    if ([self.type isEqualToString:@"1"]) {
        //初始化scrollview
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 40, self.frame.size.width, self.frame.size.height)];
    }else{
        //初始化scrollview
    ...
    //设置scrollview一开始的偏移量为一个宽度,因为里面有3个SCGIFImageView,所以scrollview默认显示的就是(0,1,2)里面的第一个imageview
    ...
    self.pageControl.pageIndicatorTintColor = [_define_black_color colorWithAlphaComponent:0.4];// 设置非选中页的圆点颜色
    
    self.pageControl.currentPageIndicatorTintColor = _define_black_color; // 设置选中页的圆点颜色
 ...
 */
- (void)setImageView:(SCGIFImageView *)imageView atIndex:(NSInteger)index
{
    if (index>_imagesArray.count-1) {
        return;
    }
    sd_downloadWebImageWithRelativePath(YES, _imagesArray[index], imageView, kStyleDetailCover,UIViewContentModeScaleAspectFill);
}
...
- (void)updateImageViewsAndPageControl {
    ...
    for (SCGIFImageView *imageView in _imageViews) {
        /*
