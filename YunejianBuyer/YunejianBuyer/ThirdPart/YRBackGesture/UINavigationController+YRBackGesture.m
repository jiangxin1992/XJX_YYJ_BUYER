...
-(void)panToBack:(UIPanGestureRecognizer*)pan{
    ...
    //NSLog(@"1xoffset %f  %f %f",xoffset,CGRectGetWidth(currentView.frame),CGRectGetMinX(currentView.frame));
    if (xoffset>0) {//向右滑
//        if (true) {
...
//        }
    ...
    //NSLog(@"2xoffset %f  %f %f",xoffset,CGRectGetWidth(currentView.frame),CGRectGetMinX(currentView.frame));
...
//            if (CGRectContainsPoint(self.view.bounds, currentView.center)) {
                ...
                if([self.topViewController isKindOfClass:[YYNewsViewController class]]){
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    if ([touch.view.superview isKindOfClass:[YYAddressCell class]] && [gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
